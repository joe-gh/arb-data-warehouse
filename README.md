...
1. **Pull** - `pull_fdm4.py` streams FDM4 `PUB` tables into an immutable
   per-run CSV directory and promotes `dump/current.json` only after the full
   production inventory has a complete manifest.
...
| Path | Purpose |
|------|---------|
| `pull_fdm4.py` | Stage 1 - JDBC pull of FDM4 `PUB` tables → `dump/run-*` + manifest |
...
- Backups are handled by AWS Backup using `infra/backup-plan.json` (the instance is
  tagged `backup=daily`). Raw/catalog data can be rebuilt from FDM4, but the
  `logo.*` operator overlay and uploaded logo media cannot. Restore the database
  and media tree to the same recovery point, then run `infra/restore-drill.sh`
  against the isolated target to produce row, invariant, and sampled-hash evidence.
