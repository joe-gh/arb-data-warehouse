-- Transform: positive-price precedence, feed-column delta hash, and a delta
-- bump on reactivation.
--
-- 1. woo.catalog_price_text() replaces three copies of the catalog-price
--    expression that each tested "> 0" only AFTER COALESCE had already chosen a
--    candidate. A colour price of 0 with a product price of 80 resolved to the
--    tier/retail price, and a missing catalog price with a tier price of 0 and
--    a retail price of 100 resolved to 0. The function returns the first
--    strictly positive candidate in the documented order (colour, product,
--    tier, retail) and returns TEXT so payloads and hashes keep the raw string.
-- 2. woo.store_product_state.feed_hash covers the 14 base columns the product
--    feed serves that are deliberately outside every payload hash (brand,
--    mill_code, category, item_name, origin_country, harmonization, ean_code,
--    def_cost, weight, design_id, design_name, color_code, size_code,
--    price_levels). Their edits changed what consumers read while row_version
--    stood still, so a caught-up consumer never fetched them again.
-- 3. row_version / changed_at now also advance when a tombstoned row comes
--    back with identical content: is_active false -> true is a state
--    transition a consumer at the old watermark would otherwise miss.
--
-- structural_hash and stockprice_hash are untouched, so the WordPress engine
-- routes exactly as before. FIRST RUN AFTER DEPLOY: feed_hash is NULL on every
-- existing row, so every row is re-versioned once and the next reconcile
-- examines the whole projection. Those rows are hash-routed on the WordPress
-- side (structural/stock-price unchanged), so no Woo writes are expected -
-- just one long examine pass.
--
-- Canonical copy: sql/woo_transform.sql (kept identical).
BEGIN;
